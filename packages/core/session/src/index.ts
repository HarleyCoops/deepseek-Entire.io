...
import type { CreateSessionOptions, EpochHeader, NonSurfaceEventIntent, PrepareSessionOptions, RequestContext, SessionEvent, SessionEventMap, SessionEventType, SessionHeader, SurfaceIntent, SurfaceEventType } from './types.ts'
...
export class Session {
   ...
   * @param opts - Surface metadata for message-producing events, or the
   *   optional `ignorable` envelope marker for log-only events. `surfaceOp`
   *   controls how an event enters the ordered surface; `sourceEventSeqs`
   *   lists the seq numbers of earlier events this one derives from. REQUIRED for
   *   {@link SurfaceEventType} events (every message-producing event must
   *   declare how it joins the surface, the sole source of derived model
   *   history), while surface metadata is rejected by the compiler for
   *   non-surface types like `turn/start` or `assistant/chunk`.
  ...
  append<T extends SessionEventType>(
    ...
    ...opts: T extends SurfaceEventType ? [opts: SurfaceIntent] : [opts?: NonSurfaceEventIntent]
  ): SessionEvent<T> {
    const eventOpts: SurfaceIntent | NonSurfaceEventIntent | undefined = opts[0]
    const eventMetadata = {
      ...eventOpts !== undefined && 'sourceEventSeqs' in eventOpts && eventOpts.sourceEventSeqs !== undefined
        ? { sourceEventSeqs: eventOpts.sourceEventSeqs }
        : {},
      ...eventOpts !== undefined && 'surfaceOp' in eventOpts && eventOpts.surfaceOp !== undefined
        ? { surfaceOp: eventOpts.surfaceOp }
        : {},
    ...
    const event = deepFreeze({
      ...
      time: Date.now(),
      data: dataSnapshot,
      ...(eventMetadataSnapshot as { ignorable?: true; surfaceOp?: unknown; sourceEventSeqs?: unknown }),
