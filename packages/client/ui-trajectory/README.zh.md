...
工具检查器会为精确的根工具调用或嵌套工具调用显示事件溯源的 Total、Policy、可选 Approval 与 Body 区段，包括已记录的结果、策略来源及中止状态。缺失端点保持未知，零时长区段保持为 `0 ms`，Chat 还会为每个调用提供始终可见的 `View tool chain` 跳转入口。
...
- **进行中时，Time 保持空白**：`partial` 与 `runningCalls` 行会显示运行状态，但不会虚构耗时，因此 Overview 区域只渲染开始标记，而不会杜撰实时跨度。
