...
The invoking directory is the default workspace root. The `web`, `headless`, and `workspace-alberta` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`.
...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-headless`, `@deepseek-ai/dsh-workspace-alberta`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
